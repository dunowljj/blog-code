...
- 요청이 있으면 그 페이지를 메모리에 올리겠다는 의미. 전부 한꺼번에 올리는게 아니고, 필요할때 page를 올린다는 의미이다.
    - I/O 양의 감소
    - (물리적)Memory 사용량 감소
    - 빠른 응답 시간 -> 관점에 따라 다를지도 system wide하게 전체적으로 생각했을 떄
    - 더 많은 사용자 수용. (멀티 프로그램)
## 1.2. Valid/Invalid
...
## 1.3. Page Fault
ex) 1번 페이지 접근하려고 봤더니 invalid? -> 해당 페이지를 디스크에서 메모리로 올려야함(I/O 작업), 사용자 프로세스가 직접 못하는 일이다. -> page fault 현상
...
- 미래에 참조되는 페이지를 모두 다 안다고 가정. 실제 시스템에서는 사용 불가. Offline algolithm
- MIN (OPT) : 가장 먼 미래에 참조되는 page를 replace
- 그림 빨간색 : page fault, 분홍색 : fault없이 메모리 직접 참조
- 4번이 가장 멀기 떄문에 처음 쫓겨났다. page default 6개가 최소
+ 아무리 좋은 알고리즘 만들어도 Optimal 못넘음. upper bound로 제공한다.
...
- Belady's Anomaly메모리를 늘렸더니 성능이 떨어지는 이상한 현상
