...
- 프로세스를 한 번에 올릴 수 없는 작은 공간의 메모리를 사용하던 초창기 시스템에서 수작업으로 프로그래머가 구현하던 것. 매우 복잡. Manual Overlay
...
- Swap in/Swap out
    ...
    - 파일 입출력과 다르게 통째로 옮겨서 양이 많다. swap time은 대부분 transfer time(swap되는 양에 비례하는 시간)임.
- 페이징할때 일부 페이지만 쫓겨나는 것 마저도 이 페이지가 swap out 되었다 라고 말하기도 한다. 원칙적인 swap은 통째로다.
...
### 4.1.2. 가변 분할 방식 (Variable Partition alloction)

- 들어갈 메모리 영역을 미리 나누어 놓지 않는것.
- 프로그램이 실행될때마다 순서대로 쌓음. 프로그램 B가 끝나면 빈공간, D는 빈공간보다 커서 아래쪽에 들어간다. 빈공간보다 커서 못들어갔으니 외부조각. 남은 공간은 외부 조각이 된다.
...
+ first-fit(오버헤드 적음)과 best-fit(적당한 홀)이 worst-fit보자 속도와 공간 이용률 측면에서 효과적인 것으로 알려짐 (실험적 결과)
...
## 4.2. Noncontiguous allocation(불연속할당)
